...
locals {
  ...
  #
  nsg_rules_runner_subnet_conditional_for_sql_private_endpoint = {
  ...
  # merge in any additional user-provided rules for the runner subnet. User supplied rules win on key collisions.
  nsg_rules_runner_subnet = merge(
    local.nsg_rules_runner_subnet_builtin,
  ...
  #   to be compatible with sql servers with 'redirect' connection policy we allow inbound on 1433-65535
  #   ref. https://learn.microsoft.com/en-us/azure/azure-sql/database/private-endpoint-overview?view=azuresql#use-redirect-connection-policy-with-private-endpoints
  nsg_rules_private_endpoint_subnet_conditional_for_sql_private_endpoint = {
  ...
  # built-in default rules for the private endpoint subnet
  nsg_rules_private_endpoint_subnet_builtin = merge(
    # when no sql pe was configured, no extra rules needed
    local.need_to_create_sql_pe ? local.nsg_rules_private_endpoint_subnet_conditional_for_sql_private_endpoint : null,
    {
      ################
      # Outbound rules
      ################
  ...
  # merge in any additional user-provided rules for the private endpoint subnet. User supplied rules win on key collisions.
  nsg_rules_private_endpoint_subnet = merge(
    local.nsg_rules_private_endpoint_subnet_builtin,
